...
The test fails when the command takes 10 seconds or more, directly enforcing RNF02 in CI. It does not modify repository content and runs with the PyYAML version pinned by the workflow. Each CI execution records the observed value as `INDEX_BUILD_1000_ELAPSED_SECONDS=<seconds>` in the unittest output.
