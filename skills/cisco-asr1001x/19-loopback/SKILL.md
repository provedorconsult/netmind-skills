---
name: 19-loopback
description: Configurar e validar Loopback para router-id, gestão, BGP e origem de serviços no Cisco ASR1001-X.
---
...
Fornecer endpoint estável sem criar exposição indevida.
...
- [INFERIDO] `interface Loopback0`
- [INFERIDO] `show ip interface brief`
- [INFERIDO] `show running-config interface Loopback0`
...
Confirmar ID livre, endereços/rotas e usos como router-id/update-source/source-interface; avaliar exposição pública e ACL.
...
Alterar loopback usada como router-id ou source derruba protocolos/gestão.

## Rollback

Restaurar endereço e dependências; não remover antes de migrar consumidores.
