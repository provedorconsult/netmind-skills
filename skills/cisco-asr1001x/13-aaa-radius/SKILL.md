---
name: 13-aaa-radius
description: Configurar e diagnosticar grupos RADIUS, autenticação, autorização e accounting no Cisco ASR1001-X sem expor shared secrets.
---
...
Integrar administração ou assinantes ao RADIUS com fallback seguro.
...
- [INFERIDO] `aaa group server radius <GROUP>`
- [INFERIDO] `radius server <NAME>`
- [INFERIDO] `aaa authentication ?`
- [INFERIDO] `aaa authorization ?`
- [INFERIDO] `aaa accounting ?`
- [INFERIDO] `show aaa servers`
- [INFERIDO] `show radius statistics`
...
Descobrir sintaxe da release; validar reachability/source-interface/VRF/ports; mascarar key; separar AAA administrativo de subscriber; testar servidor e fallback. A documentação IOS XE 17 marca a forma legada `radius-server host` como deprecated; não a usar nem a substituir sem validar a release real.
...
Shared secret, atributos e método lists errados interrompem login ou PPPoE.

## Rollback

Preservar sessão atual e fallback local; restaurar group/method list/source-interface.
