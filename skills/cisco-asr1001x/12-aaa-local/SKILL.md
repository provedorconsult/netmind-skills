---
name: 12-aaa-local
description: Planejar autenticação, autorização, accounting e usuário local de laboratório no Cisco ASR1001-X sem armazenar senhas reais.
---
...
Manter fallback administrativo seguro e testar AAA local.
...
Tratar exemplo como laboratório e `[INFERIDO][DESTRUTIVO]`; descobrir método lists; garantir sessão administrativa alternativa; testar nova sessão antes de fechar a atual.
...
AAA incorreto bloqueia todo acesso; até segredo de exemplo não deve ir à produção.

## Rollback

Registrar método lists e line bindings; restaurar configuração anterior mantendo sessão atual.
